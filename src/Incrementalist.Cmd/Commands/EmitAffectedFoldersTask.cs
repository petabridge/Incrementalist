...
namespace Incrementalist.Cmd.Commands
{
    ...
    public class EmitAffectedFoldersTask
    {
        ...
        public async Task<Dictionary<string, ICollection<string>>> Run()
        {
            ...
            var (rep, foundRepo) = GitRunner.FindRepository(Settings.WorkingDirectory);
            ...
            if (!DiffHelper.HasBranch(rep, Settings.TargetBranch))
            {
            ...
            return await filterAllFolders.Process(listAllFilesCmd.Process(Task.FromResult(rep)));
        }
    }
}